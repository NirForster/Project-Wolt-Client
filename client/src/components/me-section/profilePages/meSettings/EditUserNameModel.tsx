...
const EditUsernameModel = ({ onClose }: { onClose: () => void }) => {
  ...
  const [newfnameValue, setnewfnameValue] = useState(user?.fname);
  const [newlnameValue, setnewlnameValue] = useState(user?.lname);

  useEffect(() => {
    setCanSave(newfnameValue === user?.fname || newlnameValue === user?.lname);
  }, [newfnameValue, newlnameValue, user?.lname, user?.fname]);

  const handleSave = async () => {
    if (
      !newfnameValue ||
      !newlnameValue ||
      (newfnameValue === user?.fname && newlnameValue === user?.lname)
    )
      return;
